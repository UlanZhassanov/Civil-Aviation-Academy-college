...
@php $active_menu  = 'Докторантура' @endphp
@section('content')
    @if (session('alert'))
    ...
    @endif
    <h1>Абитуриенты на Докторантуру</h1>
    <h2>Фильтр и сортировка</h2>
    <div class="filter">
        <form action="{{ route('admin.enrollee.doctoral.index') }}">
            @csrf
            <div>
    ...
    <h2>Поиск</h2>
    <div class="filter">
        <form action="{{ route('admin.enrollee.doctoral.index') }}" method="GET">
            @csrf
            <div>
